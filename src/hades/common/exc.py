...
class SetupError(Exception):
    ...
    def __init_subclass__(cls, **kwargs: dict[str, t.Any]) -> None:
        ...
        if "exit_code" not in cls.__dict__:
            return
        if cls.__dict__["exit_code"] not in RESTART_PREVENTING_EXCEPTIONS:
            raise ValueError(
...
class UsageError(SetupError):
    exit_code = os.EX_USAGE


@contextmanager
def handle_setup_errors(logger: logging.Logger) -> t.Generator[None, None, None]:
    """If a :class:`SetupError` occurs, report it and call :func:`sys.exit` accordingly."""
    try:
        yield
