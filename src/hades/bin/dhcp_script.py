...
def generate_leasefile_lines(
        leases: Iterable[Tuple[datetime, netaddr.EUI, netaddr.IPAddress,
                               Optional[str], Optional[bytes]]]) -> str:
    """
    ...
    for expires_at, mac, ip, hostname, client_id in leases:
        ...
        yield "{expires_at:d} {mac} {ip} {hostname} {client_id}\n".format(
            ...
            mac=mac, ip=ip, hostname=hostname if hostname is not None else '*',
            client_id=client_id)
...
def obtain_and_convert(environ: Dict[str, str], name: str,
                       func: Callable[[Any], T]) -> Optional[T]:
    """
    ...
    try:
        ...
        raise ValueError("Environment variable {} contains an illegal value {}"
                         .format(name, value)) from e
...
def obtain_tuple(environ: Dict[str, str], name: str, sep: str,
                 func: Callable[[Any], T] = lambda x: x) -> Optional[Tuple[T]]:
    ...
    if value is not None:
        try:
            ...
            raise ValueError("Environment variable {} contains illegal value {}"
                             .format(name, value)) from e
    return value


def obtain_lease_info(args,
                      environ: Dict[str, str],
                      environb: Dict[bytes, bytes], *,
                      missing_as_none: bool) -> Dict[str, Any]:
    ...
    if client_id is not None:
        try:
            ...
            raise ValueError("Environment variable DNSMASQ_CLIENT_ID contains "
                             "illegal value {}"
                             .format(environ.get('DNSMASQ_CLIENT_ID'))) from e
    ...
    set_value('SuppliedHostname',
              get_env_safe(environ, 'DNSMASQ_SUPPLIED_HOSTNAME'))
    ...
    set_value('RelayIPAddress',
              obtain_and_convert(environ, 'DNSMASQ_RELAY_ADDRESS',
                                 netaddr.IPAddress))
    set_value('RequestedOptions',
              obtain_tuple(environ, 'DNSMASQ_REQUESTED_OPTIONS', ',', int))

    return values


def query_lease_for_update(connection: Connection,
                           ip: netaddr.IPAddress) -> Optional[RowProxy]:
    query = lease.select(lease.c.IPAddress == ip).with_for_update()
    with closing(connection.execute(query)) as result:
        row = result.fetchone()
        if result.fetchone() is not None:
            logger.warning("Querying database for lease with IP %s returned "
                           "more than one row", ip)
        return row


def perform_lease_update(connection: Connection, ip: netaddr.IPAddress,
                         mac: netaddr.EUI, old: RowProxy, new: Dict[str, Any]):
    changes = {k: v for k, v in new.items() if old[k] != v}
    query = lease.update(values=changes).where(lease.c.IPAddress == ip)
    result = connection.execute(query)
    if result.rowcount != 1:
        logger.warning("Unexpected row count %d while updating lease for "
                       "IP %s and MAC %s", result.rowcount, ip, mac)
    return result
...
def add_lease(args, environ: Dict[str, str], environb: Dict[bytes, bytes]):
    ...
    logger.debug("Inserting new lease for IP %s and MAC %s",
                 ip, mac)
    connection = load_config_and_connect(args)
    with connection.begin():
        ...
        if old_values is None:
            connection.execute(lease.insert(values=values))
        else:
            logger.warning("Lease for IP %s and MAC %s already exists",
                           ip, mac)
...
def delete_lease(args, environ: Dict[str, str], environb: Dict[bytes, bytes]):
    ...
    connection = load_config_and_connect(args)
    with connection.begin():
        result = connection.execute(lease.delete()
                                    .where(lease.c.IPAddress == ip))
    if result.rowcount != 1:
        logger.warning("Unexpected row count %d while deleting lease for IP %s "
                       "and MAC %s", result.rowcount, ip, mac)
...
def add_lease_command(sub_parsers, action, action_help):
    ...
    sub_parser.add_argument('mac', type=netaddr.EUI,
                            help='MAC address')
    sub_parser.add_argument('ip', type=netaddr.IPAddress,
                            help='IP address')
...
def create_parser() -> ArgumentParser:
    ...
    parser = Parser(description='dnsmasq leasefile dhcp-script to store leases '
                                'in the Hades database',
                    parents=[parent_parser])
    commands = parser.add_subparsers(metavar='COMMAND', dest='command')
    commands.required = True
    commands.add_parser('init', help='Print all leases in dnsmasq leasefile '
                                     'format')
