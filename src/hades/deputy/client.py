...
def signal_dhcp_lease_release(client_ip: netaddr.IPAddress,
                              timeout: int = 1) -> None:
    """
    Signal the deputy to release a DHCP lease.
    """
    logger.debug("Signaling DHCP lease release for IP %s on DBus: %s.%s",
                 client_ip, constants.DEPUTY_DBUS_NAME, 'Refresh')
    try:
        bus = SystemBus()
        deputy = bus.get(constants.DEPUTY_DBUS_NAME, timeout=timeout)
        deputy_interface = deputy[constants.DEPUTY_DBUS_NAME]
        deputy_interface.ReleaseDhcpLease(str(client_ip), timeout=timeout)
    except GLib.Error as e:
        handle_glib_error(e)
